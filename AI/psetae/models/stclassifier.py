"""
Modified version of Classifier proposed by Garnot et al.

In this version we added new class PseTae_finetune to allow finetuning of the initial architecture without modifying
...
class PseTae_finetune(nn.Module):
    def __init__(self, fold_folder: str, hyperparameters: dict, device: str = 'cuda'):
        """
        Pretrained PseTea classifier for fine-tuning purposes.
        The class can either load the weights of a single fold of
        weights obtained during k-fold cross-validation and produces a single prediction.
        Args:
            fold_folder (str): Path to the folder containing the fold with weights obtained during training
            hyperparameters (dict): Hyperparameters of the PseTae classifier
            device (str): Device on which the model should be loaded ('cpu' or 'cuda')
        """

        super(PseTae_finetune, self).__init__()
        self.hyperparameters = hyperparameters
        self.fold = fold_folder

        print('Loading pre-trained model . . .')
        self.model: PseTae = PseTae(**hyperparameters)
        ...
        d = torch.load(os.path.join(self.fold, 'model.pth.tar'), map_location=map_loc)

        # Filter out decoder weights
        pretrained_dict = {k: v for k, v in d['state_dict'].items() if not k.startswith('decoder')}

        self.model.load_state_dict(pretrained_dict, strict=False)

        print('Successfully loaded PseTae model instance')

        print("Freezing layers of Spatial Encoder . . .")
        for param in self.model.spatial_encoder.parameters():
            param.requires_grad = False
        print("Done!")

        print("Initializing weights for decoder . . .")
        if 'mlp4' in self.hyperparameters.keys():
            mlp4 = self.hyperparameters['mlp4']
            self.model.decoder = get_decoder(mlp4)
            self.model.decoder.apply(weight_init)
        else:
            raise ValueError("You have to provide mlp4!")
    ...
    def forward(self, input):
        ...
        Args:
            input(tuple): (Pixel-Set, Pixel-Mask) or ((Pixel-Set, Pixel-Mask), Extra-features)
                    ...
                    Extra-features : Batch_size x Sequence length x Number of features
        """
        out = self.model.spatial_encoder(input)
        out = self.model.temporal_encoder(out)
        out = self.model.decoder(out)
        return out
    
    def param_ratio(self):
        self.model.param_ratio()
