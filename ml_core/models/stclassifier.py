    ...
    def __init__(self, weight_folder, hyperparameters, device='cuda', fold='all'):
        ...
        for f in self.fold_folders:
            ...
            if device == 'cpu':
                ...
                map_loc = f'xpu:{torch.xpu.current_device()}'
                m = m.to('xpu')
                
            # Load weights onto the device specified by map_loc
            d = torch.load(os.path.join(f, 'model.pth.tar'), map_location=map_loc, weights_only=True)
            
            state_dict_with_prefix = d['state_dict']
            new_state_dict = {}
