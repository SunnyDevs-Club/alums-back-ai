...
class PixelSetData(data.Dataset):
    def __init__(self, folder, labels, npixel, sub_classes=None, norm=None,
        ...
        Args:
            ...
            return_id (bool): if True, the id of the yielded item is also returned (useful for processing)
        """
        ...
        if self.extra_feature is not None:
            ...
            # self.extra_s = np.where(self.extra_s == 0, 1e-6, self.extra_s)
            self.extra_s = np.nan_to_num(self.extra_s, nan=1.0)
