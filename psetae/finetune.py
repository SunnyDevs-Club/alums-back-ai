...
from models.stclassifier import PseTae_finetune
...
def main(config):
    ...
    if config['preload']:
        dt = PixelSetData_preloaded(config['dataset_folder'], labels='label_MoA_protoclass', npixel=config['npixel'],
                          norm=mean_std,
                          extra_feature=extra)
    else:
    ...
    for fold, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        model = PseTae_finetune(fold_folder=config['fold'], device=config['device'], hyperparameters=model_config)
